...
module.exports = class DataLakeSyncConsumer extends IterativeSQSConsumer {
	...
	async processInitialById(body, logger) {
		...
		logger.info(`[${clientCode} - ${entity} - Initial Load By Id] Starting to dump ${logRange}`);
		...
		logger.info(`[${clientCode} - ${entity} - Initial Load By Id] Finished dumping and uploaded to S3. Total items: ${processed}`);
	...
	async updateInitialLoadSettings({
		...
		const updateValues = {
			...isFirstExecution && {
				[`settings.${entity}.initialLoad.dateStart`]: executionDate,
				[`settings.${entity}.initialLoad.totalItems`]: processed
			},
			...(!isFirstExecution && processed > 0 && { $inc: { [`settings.${entity}.initialLoad.totalItems`]: processed } }),
			...(lastProcessedId && { [`settings.${entity}.initialLoad.lastId`]: lastProcessedId }),
			...(finished && { [`settings.${entity}.initialLoad.dateEnd`]: new Date() })
		};
