...
module.exports = class DataLakeSyncConsumer extends IterativeSQSConsumer {
	...
	async processSingleRecord(record, logger) {
		...
		const {
			entity, incremental, from, to, additionalFilters, limit, maxSizeMB, filenamePrefix
		} = record.body;
		...
		logger.info(`[${clientCode} - ${entity} - ${incremental ? 'Incremental' : 'Initial Load'}] Starting to dump ${from} to ${to}`);
		...
		const startNewPart = () => {
			...
			const key = `${keyPrefix}/${filenamePrefix ? `${filenamePrefix}-` : ''}${from}-${pushedAt}-${String(partIndex).padStart(3, '0')}.ndjson.gz`;
		...
		logger.info(`[${clientCode} - ${entity} - ${incremental ? 'Incremental' : 'Initial Load'}] Finished dumping and uploaded to S3. Total items: ${totalItems}`);
	}
};
