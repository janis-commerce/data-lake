...
module.exports = class DataLakeLoadFunction extends LambdaWithPayload {
	...
	async sendIncrementalLoadMessages(clientCode, now, clientSettings) {
		...
		const lastIncrementalLoadDate = clientSettings?.[this.entity]?.lastIncrementalLoadDate || this.entitySettings?.initialLoad?.dateFrom;

		if(!lastIncrementalLoadDate)
			throw new Error(`[${clientCode}] Could not find date 'from' for entity "${this.entity}" in client settings (${this.entity}.lastIncrementalLoadDate) nor in entity settings (${this.entity}.initialLoad.dateFrom)`);
