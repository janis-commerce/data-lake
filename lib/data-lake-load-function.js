...
module.exports = class DataLakeLoadFunction extends LambdaWithPayload {
	...
	async sendIncrementalLoadMessages(clientCode, now, clientEntitySettings) {
		...
		const lastIncrementalLoadDate = clientEntitySettings?.lastIncrementalLoadDate || clientEntitySettings?.initialLoad?.dateStart;
		...
		if(this.entitySettings?.initialLoad?.byId) {
			return [{
				content: {
					entity: this.entity,
					mode: 'initialById',
					...clientEntitySettings?.initialLoad?.byId && {
