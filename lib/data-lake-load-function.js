...
module.exports = class DataLakeLoadFunction extends LambdaWithPayload {
	...
	async process() {
		...
		const failedClientCodes = [];

		for(const { code, settings } of clients) {
			try {
				await this[method](code, now, settings?.[this.entity]);
			} catch(error) {
				logger.error(`[${code} - ${this.entity}] Failed to process client: ${error.message}`);
				failedClientCodes.push(code);
			}
		}

		if(failedClientCodes.length)
			throw new Error(`[${this.entity}] ${failedClientCodes.length} client(s) failed: ${failedClientCodes.join(', ')}`);
	}
	...
	async sendIncrementalLoadMessages(clientCode, now, clientEntitySettings) {
		...
		if(!lastIncrementalLoadDate) {
			logger.warn(`[${clientCode} - ${this.entity}] No initial load configured — bootstrapping incremental load from now`);
			lastIncrementalLoadDate = now;
		}
	...
	async sendInitialLoadMessages(clientCode, now, clientEntitySettings) {
		...
		if(response && !this.entitySettings?.initialLoad?.byId && this.initialLoadDateFrom) {
			return this.clientModel.update({
				[`settings.${this.entity}.initialLoad.dateFrom`]: this.initialLoadDateFrom
			}, {
				code: clientCode
			});
		}
	...
	prepareInitialLoadMessages(clientCode, clientEntitySettings) {
		...
		this.initialLoadDateFrom = new Date(dateFromCursor);
